...
export class GoToProfile {
  ...
  constructor(page: Page) {
    this.page = page;
    this.goToProfile = page.getByRole('link', { name: 'Go to profile', exact: true });
    this.createProfileBtn = page.getByRole('button', { name: 'Create profile', exact: true });

    this.myWorkBtn = page.locator('button', { hasText: 'My work' });
    this.myWorkInput = page.getByLabel('My work: ');

    this.saveBtn = page.getByRole('dialog').getByRole('button', { name: 'Save' });
    this.doneBtn = page.getByRole('button', { name: 'Done' }).nth(0);
    this.editProfileBtn = page.getByRole('button', { name: 'Edit profile', exact: true });
  }
  ...
  public async editProfile() {
    ...
    await this.doneBtn.click({ force: true });
    await expect(this.page.getByRole('button', { name: 'Create profile' })).toHaveText('Create profile');
  }
}
